...
class CALegInfoClient:
  ...
  def get_bill_digest(self, bill_id: str):
    ...
    try:
      response = requests.get(bill_text_url, headers=constants.HEADERS)
  ...
  def get_bill_status(self, bill_id: str) -> BillStatus:
    ...
    try:
      response = requests.get(bill_status_url, headers=constants.HEADERS)
      response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)
  ...
  def get_bill_list(self, session_id: str, filter: Optional[str]) -> Sequence:
    ...
    bill_list = []
    try:
      response = requests.get(bill_search_url, headers=constants.HEADERS)
      response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)
      soup = BeautifulSoup(response.content, 'html.parser')
